...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ((int)count($errors) == 0){
        ...
        // После добавления информации в БД моделируем аутентификацию
        $_SESSION['user_data'] = [];
        
        // Перенаправляем аутентифицированного пользователя на гл. страницу
        header('Location: index.php');   
    } else {
        // Если есть ошибки валидации вызываем сценарий register.php и выводим их под полями
        require_once('functions.php');
        $register_content = include_template('register.php', [ 'user_value' => $user_value, 'errors' => $errors ]);                                     print($register_content);    
    }
}
